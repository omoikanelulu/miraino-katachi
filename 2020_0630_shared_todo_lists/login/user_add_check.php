...
try {
    ...
    Security::session();
    if (!Security::matched_token($_POST['token'])) { // falseだったら不正なアクセス
        $_SESSION['err']['msg'] = '不正なアクセスです';
        header('Location: ../error/error.php');
        exit();
    }

    // session_start()してからでないと、$_SESSIONのunsetが出来なかった
    ...
    if ($hasErr == true) {
        // Security::makeToken();
        header('location:./user_add.php', true, 307);
        exit();
    }

    $_SESSION['data']['is_admin'] == 1 ? $is_admin = 'checked="checked"' : $is_admin = '';
