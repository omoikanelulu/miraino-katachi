...
try {
    ...
    if (isset($_SESSION['data']) == true) {
        isset($_SESSION['data']['id']) ? $id = $_SESSION['data']['id'] : $id = '';
    ...
    if (empty($_SESSION['data'])) {
        $userIns = new Users;
        $userInfos = $userIns->pick_one($_POST['id']); // ユーザ情報を取得
        isset($userInfos['id']) ? $id = $userInfos['id'] : $id = '';
        isset($userInfos['user']) ? $user = $userInfos['user'] : $user = '';
        isset($userInfos['family_name']) ? $family_name = $userInfos['family_name'] : $family_name = '';
        isset($userInfos['first_name']) ? $first_name = $userInfos['first_name'] : $first_name = '';
        // 管理者権限を持っていたらチェックを入れるようにする
        isset($userInfos['is_admin']) && $userInfos['is_admin'] == 1 ? $is_admin = 'checked="checked"' : $is_admin = '';
        // 削除フラグが立っていたらチェックを入れるようにする
        isset($userInfos['is_deleted']) && $userInfos['is_deleted']== 1 ? $is_deleted = 'checked="checked"' : $is_deleted = '';
    }
        ...
        <div class="col-sm-6">
            <form action="./user_edit_check.php" method="post">
                ...
                <div class="form-group form-check">
                    ...
                    <input type="checkbox" value="1" <?= $is_deleted ?> class="form-check-input" id="is_deleted" name="is_deleted">
                    <label for="is_deleted">ユーザを削除する</label>
                </div>
                <br>
                <button type="submit" class="btn btn-primary">更新</button>
                <input type="button" value="キャンセル" class="btn btn-outline-primary" onclick="location.href='./user_index.php';">
            </form>
        </div>
        <div class="col-sm-3"></div>
    </div>
