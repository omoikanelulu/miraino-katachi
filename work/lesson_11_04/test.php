...
try {
    ...
    // SQLでDBエンジンに指示
    $sql = 'SELECT * FROM todo_items ORDER BY todo_items.expiration_date ASC';
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php foreach ($list as $key) : ?>
                            <tr>
                                <td class="<?php if ($key['is_completed'] == 1) echo 'comp_line' ?>"><?= $key['expiration_date'] ?></td>
                                <td class="<?php if ($key['is_completed'] == 1) echo 'comp_line' ?>"><?= $key['todo_item'] ?></td>
                                <form action="./action.php" method="POST">
                                    <td><input type="hidden" name="id" value="<?= $key['id'] ?>"></td>
                                    <td><label><input type="radio" name="is_completed" value="0" <?php if ($key['is_completed'] == 0) echo 'checked' ?>>未完了</label></td>
                                    <td><label><input type="radio" name="is_completed" value="1" <?php if ($key['is_completed'] == 1) echo 'checked' ?>>完了</label></td>
